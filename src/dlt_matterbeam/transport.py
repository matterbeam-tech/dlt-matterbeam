"""The `MatterbeamTransport` seam.

Everything above this line -- envelope construction, key extraction, disposition
translation, PUA stripping -- is transport-agnostic. Below it, two builtin
implementations: `FileTransport` writes plain newline-delimited JSON to a local
directory with no server, for local debugging only, so it makes no attempt to
reproduce the real internal coldlog wire format. `HttpTransport` talks to a real
Matterbeam account's REST API. An internal-only optional package can add a
`DirectColdlogTransport` using the real `ColdlogWriter` in-process; registration, state,
and the ingest write are all on this Protocol rather than only in `HttpTransport` so
that an in-process transport can answer them directly, with no network call at all.

Discovery mirrors dlt's own plugin resolution: a transport is looked up by name, and
...
package directly -- it doesn't know it exists. `tests/unit/test_transport_seam.py`
asserts that holds with the internal package absent, which is the only way this seam
stays honest as the codebase changes.
"""
...
class Conflict(Exception):
    ...
    `ingest`/`register`/`complete_load` should turn this into a terminal job failure (a
    closed load or a paused pid really is terminal); `put_dlt_state` should not -- a
    stale-state conflict is a bookkeeping race, not a reason to fail an otherwise-good
    data load (two pipeline instances writing state concurrently can 409 on the state
    write alone, after the row data has already landed correctly).
    """
...
class MatterbeamTransport(Protocol):
    ...
    `FileTransport` -- that is a legitimate, silently-degrading `WithStateSync` gap, not
    an error.
    """
...
class FileTransport:
    """Writes plain newline-delimited JSON to a local directory, purely for local
    debugging -- there is no real Matterbeam account behind it, so it makes no attempt
    to reproduce the real internal coldlog wire format (no CRF framing, no compression,
    no record_id allocation). One human-readable `<recordtype_id>.jsonl` file per table,
    appended to across runs. No Matterbeam imports, no network, no server-side identity
    -- `register`/state are no-ops, degrading silently exactly like `WithStateSync` not
...
class HttpTransport:
    """Talks to the customer REST API. The server allocates `record_id`s and stamps
    `mb.metadata` -- this transport only builds the minimal wire envelope (`{"t": bool,
    "i": str|null, "v": {...}}`; the fold-key field is dropped, not replaced, since the
    fold-key declaration path is out of scope for this transport) and handles the two
    distinguishable 409s: lock contention is retried with backoff, a closed load or a
    paused pid is terminal.
    """
    ...
    def put_dlt_state(self, pid: str, doc: dict) -> None:
        """Guarded on `StateInfo.version` server-side. A stale write loses this race
        against a newer one and is logged, not raised -- dlt's own bookkeeping losing a
        race is not a reason to fail a load whose *rows* already landed."""
        try:
    ...
    def send_chunk(
        ...
        *,
        ...
        URL -- a pid is already the identity a dlt pipeline registers as, so
        `dataset_name` would be a second, redundant axis of identity, and collides with
        Matterbeam's own unrelated `Dataset` concept.
        """
        import zstandard
